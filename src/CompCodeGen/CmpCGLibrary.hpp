...
namespace CmpCG {
    ...
    const std::optional<inja::json> ArgParseLib(const cxxopts::ParseResult& result) noexcept;
    
    /// @brief プロパティとテンプレートを元にコードを生成する
    /// @param prop 生成に用いるプロパティ
    /// @param out 出力先ストリーム
    void TplPathLib(const inja::json& prop, const cxxopts::ParseResult& result, std::ostream& out) noexcept;
};